...
export const productFeedbackSlice = createSlice({
  ...
  reducers: {
    ...
    toggleVote: (state, action: PayloadAction<string>) => {
      ...
      const selectedProduct = state.suggestions.find(
      ...
      if (selectedProduct) {
        ...
        if (selectedProduct.upvoted.includes(username)) {
          state.suggestions[index].upvoted = selectedProduct.upvoted.filter(
            (name) => name !== username
          );
    ...
    deleteProduct: (state, action: PayloadAction<string>) => {
      ...
      state.roadmap = state.roadmap.filter((product) => product.id !== id);
